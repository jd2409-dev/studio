  ...
  return (
    <div className="container mx-auto py-8">
      ...
      <p className="text-muted-foreground mb-8">
        ...
        <br/>The AI will generate text, audio script, and mind map summaries.
      </p>
        ...
        <Card className="shadow-md rounded-lg">
          ...
          <form onSubmit={handleSubmit}>
            <CardContent className="space-y-4">
              ...
              {file && (
                <div className="mt-4 border rounded-md overflow-hidden min-h-[200px] max-h-96 flex flex-col justify-center items-center bg-muted p-4 text-center">
                 {isImageFile && previewUrl ? (
                     <Image
                        ...
                        className="w-auto h-auto max-w-full max-h-[280px] object-contain" // Adjusted max height further
                        data-ai-hint="textbook page"
                      />
                  ) : (
                     <div className="flex flex-col items-center justify-center text-center">
                        <FileTextIcon className="h-16 w-16 text-muted-foreground mb-2" />
                        <p className="text-sm font-medium" title={file.name}>{file.name}</p>
          ...
          <CardContent className="min-h-[300px]"> {/* Ensure minimum height */}
            ...
            {summary ? (
              <Tabs defaultValue="text" className="w-full">
                ...
                <TabsContent value="text" className="mt-2 p-4 border rounded-md bg-muted/30 min-h-[200px] max-h-[400px] overflow-y-auto prose prose-sm dark:prose-invert max-w-none">
                  <h3 className="font-semibold mb-2 text-base">Text Summary</h3>
                   <p className="whitespace-pre-wrap">{summary.textSummary}</p>
                </TabsContent>
                <TabsContent value="audio" className="mt-2 p-4 border rounded-md bg-muted/30 min-h-[200px] max-h-[400px] overflow-y-auto prose prose-sm dark:prose-invert max-w-none">
                   <h3 className="font-semibold mb-2 text-base">Audio Summary (Script)</h3>
                      <>
                       <p className="whitespace-pre-wrap">{summary.audioSummary}</p>
                       <p className="text-xs text-muted-foreground mt-2">
                           This is a script suitable for text-to-speech. You can copy this text and use a TTS service or browser extension to generate audio.
                       </p>
                       </>
                 </TabsContent>
                <TabsContent value="mindmap" className="mt-2 p-4 border rounded-md bg-muted/30 min-h-[200px] max-h-[400px] overflow-y-auto">
                  <h3 className="font-semibold mb-2 text-base">Mind Map (Markdown)</h3>
                     <>
                       <pre className="text-sm whitespace-pre-wrap font-mono bg-background p-3 rounded-md border">{summary.mindMap}</pre>
                       <p className="text-xs text-muted-foreground mt-2">Mind map structure in Markdown format.</p>
                     </>
                </TabsContent>
              </Tabs>
            ) : (
               !isLoading && <p className="text-muted-foreground text-center h-full flex items-center justify-center py-10">Upload a file and click "Generate Summaries" to see the results.</p>
            )}
          </CardContent>
           <CardFooter className="pt-4 border-t">
              {summary && <Button variant="outline" size="sm" onClick={handleClear}>Clear Summary</Button>}
           </CardFooter>
        </Card>
      </div>
    </div>
